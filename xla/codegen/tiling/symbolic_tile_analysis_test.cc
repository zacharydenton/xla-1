...
ENTRY main {
  ...
  ASSERT_EQ(symbolic_computation.size(), 1);
  const HloInstruction* dot_hlo =
      module->entry_computation()->root_instruction()->fused_expression_root();

  const SymbolicTiledHloInstruction* symbolic_dot = nullptr;
  // TODO: that can be simplified as the only instruction is the dot.
  for (const auto& instruction : analysis->GetSymbolicTiledHloComputation()) {
    if (instruction->hlo() == dot_hlo) {
      symbolic_dot = instruction.get();
      break;
    }
  }
  ASSERT_NE(symbolic_dot, nullptr);
  EXPECT_EQ(symbolic_dot->regions().front().size(), 3);
  ASSERT_EQ(symbolic_dot->operands().size(), 2);
...
ENTRY main {
  ...
  std::optional<SymbolicTileAnalysis> analysis = TryAnalyzeModule(module.get());
  ASSERT_TRUE(analysis.has_value());
  ASSERT_EQ(analysis->GetSymbolicTiledHloComputation().size(), 1);
  const auto& concat = analysis->GetSymbolicTiledHloComputation().front();
  ASSERT_NE(concat, nullptr);
  EXPECT_EQ(concat->hlo()->opcode(), HloOpcode::kConcatenate);
  ASSERT_EQ(concat->operands().size(), 3);
  ASSERT_EQ(concat->regions().size(), 3);
...
ENTRY main {
  ...
  const TiledHloInstruction* concat = tiled_hlo_computation.GetRoots()[0];
...
ENTRY main {
  ...
  ASSERT_THAT(tiled_hlo_computation_or,
              absl_testing::StatusIs(
                  absl::StatusCode::kUnimplemented,
                  ::testing::HasSubstr("not divisible by tile size")));
}
