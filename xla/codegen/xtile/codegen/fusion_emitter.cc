...
// Returns whether we expect to see sub-regions defined for
// TiledHloInstruction.
...
absl::Status EmitTiledInstructionList(
  ...
  for (const std::unique_ptr<TiledHloInstruction>& tiled_hlo :
    ...
    TF_ASSIGN_OR_RETURN(
        TensorValue result,
        EmitTiledHloInstruction(b, fusion, *tiled_hlo.get(), fn, pid, values));
...
absl::StatusOr<TensorValue> EmitUnnestedDot(
  ...
  //
  // To emit it we create a loop over the contracting dimension and emit
  // the the region of the dot inside:
  //
