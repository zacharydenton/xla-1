...
bool ShouldRewrite(const HloDotInstruction* dot, const OneHotMatch& match) {
  ...
  if (depth < 256 && ShapeUtil::ElementsIn(match.indices->shape()) > 1024) {
    return false;
  }
  return true;
}
...
std::optional<OneHotMatch> TryMatchOneHotDot(HloInstruction* instr) {
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    // sometimes wrapped in a call to a custom call.
    if (Match(root, m::Compare())) {
      compare = root;
    } else if (root->opcode() == HloOpcode::kCall) {
      call_op = root;
      HloInstruction* call_root = call_op->to_apply()->root_instruction();
      HloInstruction* op;
      while (Match(call_root, m::Convert(m::Op(&op)))) {
        call_root = op;
      }
      if (call_root->opcode() == HloOpcode::kCompare) {
        compare = call_root;
      }
    }
    ...
    if (found_indices) {
      HloInstruction* final_indices = found_indices;
      if (call_op) {
        if (found_indices->opcode() == HloOpcode::kParameter) {
          final_indices =
              call_op->mutable_operand(found_indices->parameter_number());
        }
      }
      ...
      if (final_indices) {
        ...
        if (final_indices->shape().dimensions().size() +
                weights->shape().dimensions().size() - 1 !=
            dot->shape().dimensions().size()) {
          continue;
        }
        OneHotMatch match{final_indices, weights, i == 0};
...
absl::StatusOr<bool> OneHotGatherRewriter::RunImpl(
  ...
  for (HloComputation* computation :
    ...
    for (HloInstruction* instr : computation->MakeInstructionPostOrder()) {
      if (auto match = TryMatchOneHotDot(instr)) {
        RETURN_IF_ERROR(RewriteOneHotDotToGather(
            computation, Cast<HloDotInstruction>(instr), *match));
        changed = true;
      }
    }
  }
