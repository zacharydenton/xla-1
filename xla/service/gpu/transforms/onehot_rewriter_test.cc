...
one_hot_computation (indices_param: s32[2,2,2]) -> pred[2,2,2,8] {
  %indices_param = s32[2,2,2]{2,1,0} parameter(0)
  %indices_r1 = s32[2,2,2,1]{3,2,1,0} reshape(%indices_param)
  ...
  ROOT %compare = pred[2,2,2,8]{3,2,1,0} compare(%indices_b_final, %iota_b_final), direction=EQ
}

ENTRY %main {
  %indices = s32[2,2,2]{2,1,0} parameter(0)
  %one_hot = pred[2,2,2,8]{3,2,1,0} call(%indices), to_apply=one_hot_computation
...
TEST_F(OneHotGatherRewriterTest, Positive_ComplexTrace) {
  absl::string_view hlo_string = R"(
HloModule Positive_ComplexTrace

ENTRY Main {
  indices = s32[] parameter(0)
  r1 = s32[1] reshape(indices)
  r2 = s32[] reshape(r1)
  c1 = s32[] convert(r2)

  c2_broadcast = s32[10] broadcast(c1), dimensions={}
  iota = s32[10] iota(), iota_dimension=0
  compare = pred[10] compare(c2_broadcast, iota), direction=EQ
...
TEST_F(OneHotGatherRewriterTest, CallMapping) {
  absl::string_view hlo_string = R"(
HloModule CallMapping

one_hot_sub (p0: s32[]) -> f32[10] {
  p0 = s32[] parameter(0)
  p0_broadcast = s32[10] broadcast(p0), dimensions={}
  iota = s32[10] iota(), iota_dimension=0
  compare = pred[10] compare(p0_broadcast, iota), direction=EQ
  ROOT result = f32[10] convert(compare)
}

ENTRY Main {
  indices = s32[] parameter(0)
  one_hot = f32[10] call(indices), to_apply=one_hot_sub
  weights = f32[10, 128] parameter(1)
  ROOT dot = f32[128] dot(one_hot, weights),
             lhs_contracting_dims={0}, rhs_contracting_dims={0}
}
