...
TEST(HloShardingUtilTest, MergeNamedShardingIfCompatibleCompatible) {
  Mesh mesh({2, 2}, {"x", "y"});
  HloSharding sharding1(test_utils::FromAxisNames(mesh, {{"x"}, {}}));
  HloSharding sharding2(test_utils::FromAxisNames(mesh, {{}, {"y"}}));

  EXPECT_TRUE(MergeNamedShardingIfCompatible(sharding1, &sharding2));
  EXPECT_EQ(sharding2.named_sharding(),
            test_utils::FromAxisNames(mesh, {{"x"}, {"y"}}));
}
...
TEST(HloShardingUtilTest, MergeShardingIfCompatibleNamedShardingCompatible) {
  ...
  EXPECT_TRUE(MergeShardingIfCompatible(sharding1, &sharding2));
  EXPECT_EQ(sharding2,
            HloSharding(test_utils::FromAxisNames(mesh, {{"x"}, {"y"}})));
}
...
TEST(HloShardingUtilTest, MergeNamedShardingIfCompatibleSame) {
  Mesh mesh({2, 2}, {"x", "y"});
  HloSharding sharding1(test_utils::FromAxisNames(mesh, {{"x"}, {"y"}}));
  HloSharding sharding2 = sharding1;

  EXPECT_TRUE(MergeNamedShardingIfCompatible(sharding1, &sharding2));
  EXPECT_EQ(sharding2, sharding1);
}

TEST(HloShardingUtilTest, MergeNamedShardingIfCompatibleOverlap) {
  Mesh mesh({2, 2, 2}, {"x", "y", "z"});
  HloSharding sharding1(test_utils::FromAxisNames(mesh, {{"x", "y"}}));
  HloSharding sharding2(test_utils::FromAxisNames(mesh, {{"y", "z"}}));

  EXPECT_FALSE(MergeNamedShardingIfCompatible(sharding1, &sharding2));
}

TEST(HloShardingUtilTest, MergeNamedShardingIfCompatiblePrefix1) {
  Mesh mesh({2, 2, 2}, {"x", "y", "z"});
  HloSharding sharding1(test_utils::FromAxisNames(mesh, {{"x", "y"}}));
  HloSharding sharding2(test_utils::FromAxisNames(mesh, {{"x"}}));
...
TEST(HloShardingUtilTest, MergeNamedShardingIfCompatiblePrefix2) {
  Mesh mesh({2, 2, 2}, {"x", "y", "z"});
  HloSharding sharding1(test_utils::FromAxisNames(mesh, {{"x"}}));
  HloSharding sharding2(test_utils::FromAxisNames(mesh, {{"x", "y"}}));
...
TEST(HloShardingUtilTest, MergeNamedShardingIfCompatibleIncompatibleMesh) {
  Mesh mesh1({2}, {"x"});
  Mesh mesh2({2}, {"y"});
  HloSharding sharding1(test_utils::FromAxisNames(mesh1, {{"x"}}));
  HloSharding sharding2(test_utils::FromAxisNames(mesh2, {{"y"}}));
