...
bool AxisRefLess(const AxisRef& a, const AxisRef& b) {
...
std::vector<AxisRef> GetSortedAxes(absl::Span<const AxisRef> axes) {
  ...
  std::sort(sorted.begin(), sorted.end(), AxisRefLess);
  return sorted;
}
...
MergeDimensionShardings(const NamedSharding& src_named,
  ...
  for (int64_t i = 0; i < rank; ++i) {
    ...
    absl::Span<const AxisRef> merged_axes;
    ...
    for (const AxisRef& axis : merged_axes) {
    ...
    merged_dim_shardings.emplace_back(
        std::vector<AxisRef>(merged_axes.begin(), merged_axes.end()),
        is_closed);
  }
...
std::vector<AxisRef> MergeReplicatedAxes(
  ...
  std::sort(candidates.begin(), candidates.end(), AxisRefLess);
...
bool MergeNamedShardingIfCompatible(const HloSharding& to_merge,
  ...
  if (src_named.mesh() != dst_named.mesh() ||
      src_named.dim_shardings().size() != dst_named.dim_shardings().size()) {
    return false;
  }
